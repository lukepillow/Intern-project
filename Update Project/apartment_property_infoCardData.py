...
def getInfo(listingID):
	'''Takes a string, listingID, and returns the infoCardData request for that ID.'''
	...
	try:
		...
		time.sleep(1)
		return getInfo(listingID)
	#return req.json()
	return req


def parseInfo(infoJSON):
	'''Takes a json from the apartments.com infoCardData api and returns a tuple of the desired features.'''
