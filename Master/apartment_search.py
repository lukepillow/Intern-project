...
payload = {"Map":{"BoundingBox":{"UpperLeft":{"Latitude":0,"Longitude":0},"LowerRight":{"Latitude":0,"Longitude":0}},"CountryCode":"US"},"Geography":{"GeographyType":7,"Address":{},"Location":{"Latitude":33.81202,"Longitude":-116.3889}},"Listing":{},"Paging":{"Page":None},"ResultSeed":558390,"Options":1,"IsBoundedSearch":None}


def getResponse(upperLeftCoordinates, lowerRightCoordinates, page = 1):
	'''Takes a tuple containing the lat, lon coordinates for the upper left and lower right corners of a box.
	Returns a json containing the search data for that box.'''
...
def getNumberOfListings(responseJSON):
	#NOTE, THIS ONLY RETURNS NUMBER OF UNITS IN ACTIVE LISTINGS!!!
...
def getPinData(responseJSON):
	...
	for i in range(0, len(dataList)-1, 5):
		...
		# This should be 99.8% of cases
		if length == 9:
			# Check for if multiple listings are at same pin
